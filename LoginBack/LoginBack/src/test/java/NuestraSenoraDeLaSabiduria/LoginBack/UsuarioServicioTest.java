...
public class UsuarioServicioTest {
    ...
    @BeforeEach
    void setUp() {
        estudiante = new Estudiante.Builder("usuarioEstudiante", "password123", "Juan Pérez")
        ...
        responsable = ResponsableEconomico.builder()
    ...
    void testRegistrarResponsableCuandoNoExiste() {
        when(responsableEconomicoRepository.existsByCorreoElectronico(responsable.getCorreoElectronico()))
                .thenReturn(false);
        when(responsableEconomicoRepository.save(responsable)).thenReturn(responsable);

        ResponsableEconomico resultado = usuarioServicio.registrarResponsable(responsable);
        assertNotNull(resultado);
    ...
    void testRegistrarBibliotecario() {
        ...
        assertNotNull(resultado);
        assertEquals(bibliotecario.getNombreUsuario(), resultado.getNombreUsuario());
    ...
    void testLoginUsuarioConCredencialesValidas() throws Exception {
        ...
        Usuario resultado = usuarioServicio.loginUsuario("usuarioEstudiante", "password123");
        assertNotNull(resultado);
    ...
    void testValidarResponsableCuandoExiste() {
        when(responsableEconomicoRepository.existsByCorreoElectronico("carlos.lopez@example.com"))
                .thenReturn(true);
